 ...
 *  TcpConnection是muduo中最核心最复杂的类，主要包含一个Channel和一个Socket数据成员，包含读事件、写事件、关闭连接等事件的回调函数
 */
...
private:
    ...
    std::unique_ptr<Socket> socket_;    // 这个连接对应的socket
    ...
    MessageCallback messageCallback_;       // 有读写消息时的回调   在handleread函数中被调用

...
/**
 *  eventfd 是一个计数相关的fd。
 *  计数不为零是有可读事件发生，read 之后计数会清零，write 则会递增计数器。
...
void EventLoop::loop()
{
    ...
    while(!quit_)
    {
         ...
         *  唤醒(wakeup) subLoop后，执行下面的方法，执行之前mainLoop注册的cb操作
         */
        doPengingFunctors();
    }
 ...
 *  2. 由于doPendingFunctors()调用的Functor可能再次调用queueInLoop(cb)，这时queueInLoop()就必须wakeup(),
 *     否则新增的cb可能就不能及时调用了
